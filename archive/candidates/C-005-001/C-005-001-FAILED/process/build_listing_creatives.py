...
RECORD = BASE / "records/listing_creatives/LC-C-005-001.yaml"
...
def creative_svg(spec: dict[str, str]) -> str:
  ...
  <rect x="850" y="190" width="940" height="910" rx="28" fill="#eef3f5" stroke="{BORDER}" stroke-width="3"/>
  <rect x="886" y="230" width="868" height="830" rx="18" fill="#ffffff" stroke="{BORDER}" stroke-width="2"/>
  <clipPath id="proofClip">
    <rect x="906" y="250" width="828" height="790" rx="12"/>
  </clipPath>
  <image x="906" y="250" width="828" height="790" preserveAspectRatio="xMidYMin slice" clip-path="url(#proofClip)" href="{image}"/>
...
def record_yaml() -> str:
    lines = [
        ...
        "  candidate_id: C-005-001",
        "  listing_hook_ref: records/listing_specs/LS-C-005-001.yaml",
        "  passed_qa_ref: records/validation/RAI-C-005-001.yaml",
        "  founder_acceptance_ref: records/blind_buyer_walkthrough/BBW-C-005-001.yaml",
        "  source_surface_refs:",
    ]
    for spec in CREATIVES:
        lines.append(f"    - builds/C-005-001/listing-assets/real-sheet-previews/{spec['source']}")
    lines.extend(["  creative_assets:"])
    for spec in CREATIVES:
        lines.extend([
            f"    - path: builds/C-005-001/listing-assets/final/{spec['file']}.png",
            f"      source_svg: builds/C-005-001/process/listing-creatives/svg/{spec['file']}.svg",
            f"      role: {spec['role']}",
            f"      source_surface_ref: builds/C-005-001/listing-assets/real-sheet-previews/{spec['source']}",
            f"      buyer_promise: {spec['title']}",
            f"      grid_stop_reason: {spec['grid_reason']}",
            f"      reason_to_believe: {spec['claim']}",
            f"      proof_surface: builds/C-005-001/listing-assets/real-sheet-previews/{spec['source']}",
    ...
    for spec in CREATIVES:
        lines.extend([
            f"    - claim: {spec['claim']}",
            f"      creative_path: builds/C-005-001/listing-assets/final/{spec['file']}.png",
            "      proof_ref: records/validation/RAI-C-005-001.yaml",
            f"      source_surface_ref: builds/C-005-001/listing-assets/real-sheet-previews/{spec['source']}",
            f"      grid_stop_reason: {spec['grid_reason']}",
        ])
    lines.extend([
    ...
    for spec in CREATIVES:
        lines.append(f"    - builds/C-005-001/listing-assets/final/{spec['file']}.png")
    lines.extend([
