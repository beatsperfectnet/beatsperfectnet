...
BUILD = BASE / "builds/C-004-001-R2"
...
RECORD = BASE / "records/listing_creatives/LC-C-004-001-R2.yaml"
...
def record_yaml() -> str:
    lines = [
        ...
        "  listing_hook_ref: records/listing_hook/LH-C-004-001-R2.yaml",
        "  passed_qa_ref: records/validation/QA-C-004-001-R2.yaml",
        "  founder_acceptance_ref: records/validation/FA-C-004-001-R2.yaml",
        "  source_surface_refs:",
    ]
    for spec in CREATIVES:
        lines.append(f"    - builds/C-004-001-R2/listing-assets/real-sheet-previews/{spec['source']}")
    lines.extend(["  creative_assets:"])
    for spec in CREATIVES:
        lines.extend([
            f"    - path: builds/C-004-001-R2/listing-assets/final/{spec['file']}.png",
            f"      source_svg: builds/C-004-001-R2/process/listing-creatives/svg/{spec['file']}.svg",
            f"      role: {spec['role']}",
            f"      source_surface_ref: builds/C-004-001-R2/listing-assets/real-sheet-previews/{spec['source']}",
            f"      buyer_promise: {spec['title']}",
            f"      grid_stop_reason: {spec['grid_reason']}",
            f"      reason_to_believe: {spec['claim']}",
            f"      proof_surface: builds/C-004-001-R2/listing-assets/real-sheet-previews/{spec['source']}",
    ...
    for spec in CREATIVES:
        lines.extend([
            f"    - claim: {spec['claim']}",
            f"      creative_path: builds/C-004-001-R2/listing-assets/final/{spec['file']}.png",
            "      proof_ref: records/validation/QA-C-004-001-R2.yaml",
            f"      source_surface_ref: builds/C-004-001-R2/listing-assets/real-sheet-previews/{spec['source']}",
            f"      grid_stop_reason: {spec['grid_reason']}",
        ])
    lines.extend([
    ...
    for spec in CREATIVES:
        lines.append(f"    - builds/C-004-001-R2/listing-assets/final/{spec['file']}.png")
    lines.extend([
